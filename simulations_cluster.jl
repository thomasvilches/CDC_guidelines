...
function create_folder(ip::cv.ModelParameters,vac="none")
    ...
    main_folder = "/data/thomas-covid/US_April"
    #main_folder = "."
    ...
    RF = string(main_folder,"/results_prob_","$(replace(string(ip.β), "." => "_"))","_herd_immu_","$(ip.herd)","_$vac","_$(ip.third_strain_trans)_$(ip.file_index)") ##  
...
function run_param_scen(b,h_i = 0,ic=1,fs=0.0,fm=0.0,strain2_trans=1.5,vac = "none",red = 0.0,index = 0,tr=999,when_= 999,dosis=3,ta = 999,nsims=500)
    ...
    #b = bd[h_i]
    #ic = init_con[h_i]
    @everywhere ip = cv.ModelParameters(β=$b,fsevere = $fs,fmild = $fm,vaccinating = true,vac_efficacy_inf = $inf,
    vac_efficacy_symp=$symp, vac_efficacy_sev = $sev,
